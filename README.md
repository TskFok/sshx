...
- 内置终端：xterm.js，多标签会话
- 文件传输：本地/远程双栏浏览，单文件上传下载，传输进度与历史记录
- 连接测试：保存前可先验证连接可用性
- 断线重连：会话断开后按回车快速重连
...
- SSH 终端、文件传输和连接测试首次遇到未知主机时，会在应用内展示主机、端口、密钥算法及 SHA256 指纹。通过服务器管理员等独立渠道核验指纹后，勾选核验确认并点击「信任并连接」，即可保存主机记录并继续连接，无需另开系统终端。取消或超时不会保存信任。已有记录不符或密钥已撤销时仍会拒绝连接，不提供直接覆盖入口；请先核实变更原因。macOS 使用系统 OpenSSH 的主机记录和原生校验，其他平台使用用户的 `~/.ssh/known_hosts`。
