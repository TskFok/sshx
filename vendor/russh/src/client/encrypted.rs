                    ...
                    b"keepalive@openssh.com" => {
                        ...
                        if wants_reply == 1 {
                            if let Some(ref mut enc) = self.common.encrypted {
                                ...
                                // RFC 4254: recipient channel is the server's channel number for
                                // packets client → server (our `recipient_channel`), not local id.
                                if let Some(params) = enc.channels.get(&channel_num) {
                                    push_packet!(enc.write, {
                                        enc.write.push(msg::CHANNEL_SUCCESS);
                                        enc.write.push_u32_be(params.recipient_channel)
                                    });
                                }
                            }
                        } else {
    ...
    pub(crate) fn write_auth_request_if_needed(&mut self, user: &str, meth: auth::Method) -> bool {
        ...
        if let Some(ref mut enc) = self.common.encrypted {
            is_waiting = match enc.state {
                EncryptedState::WaitingAuthRequest(ref mut auth_request) => {
                    // After USERAUTH_FAILURE, the next USERAUTH_REQUEST (e.g. keyboard-interactive)
                    // must set `current` so message 60 is parsed as INFO_REQUEST, not discarded.
                    // Otherwise `current` stays None and the KI prompts never reach the handler
                    // (see JumpServer / partial publickey success then MFA).
                    match &meth {
